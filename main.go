...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	return ((CaloriesWeightMultiplier*w.Weight + (math.Pow(w.meanSpeed(), 2)/float64(w.Height))*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
}
...
func (s Swimming) TrainingInfo() InfoMessage {
	...
	InfoMessage.Speed = float64(s.LengthPool) * float64(s.CountPool) / MInKm / s.Duration.Hours()
	return InfoMessage
}
